...
},
{
    ...
    items: ['1', '4', '16', '30'],
    answer: 2,
    annotation: 'В течение дня восход солнца на орбите можно наблюдать 16 раз, так как каждые 90 минут МКС совершает виток вокруг планеты.'
},
{
...
async function newGame() {
    ...
    tl = 0;
    endGame = false;
    block.innerHTML = `<div class="questions__item__text"></div>
                            <div class="questions__start">Начать тест!</div>`;
}
...
block.addEventListener('click', async (e) => {
    if (e.target.classList.contains('question__item__options__item')) {
        ...
        await checkLose();
        if(questions[tour].annotation) {
            await showAnnotation(questions[tour].annotation);
        } else {
            await nextQuestion(tour + 1);
        }
        await tour++;
    ...
    if (e.target.classList.contains('questions__next')) {
        ...
        await nextQuestion(tour);
        await gsap.to(block, {
            opacity: 1,
            duration: 0.5,
        })
    } else if (e.target.classList.contains('questions__check')) {
        await letsGo();
        await gsap.to(block, {
            opacity: 0,
            duration: 0.2,
...
function nextQuestion(number) {
    if (tour == 10 && !endGame) {
        block.innerHTML = `<div class="questions__item__text">Кажется, ракета готова к запуску...</div>
                            <div class="questions__check">ПОЕХАЛИ!</div>`;
    } else if (mistakes >= 6 && endGame == true) {
        ...
        if (questions[number].items.length == 4) {
            block.innerHTML = `<div class="questions__item__text">${questions[number].question}</div>
        <ul class="questions__item__options">
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[0]}</div></li>
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[1]}</div></li>
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[2]}</div></li>
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[3]}</div></li>
        </ul>`;
        } else {
            block.innerHTML = `<div class="questions__item__text">${questions[number].question}</div>
        <ul class="questions__item__options">
            ...
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[2]}</div></li>
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[3]}</div></li>
            <li class="questions__item__options__wrapper"><div class="question__item__options__item">${questions[number].items[4]}</div></li>
        </ul>`;
        }
